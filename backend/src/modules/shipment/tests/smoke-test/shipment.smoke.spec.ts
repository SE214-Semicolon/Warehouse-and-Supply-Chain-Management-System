...
const TEST_SUITE_ID = `ship-smoke-${Date.now()}-${Math.random().toString(36).substring(7)}`;

describe('Shipment Module - Smoke Tests', () => {
  ...
  beforeAll(async () => {
    ...
    const logistics = await prisma.user.create({
      data: {
        username: `logistics-smoke-${TEST_SUITE_ID}`,
        email: `logistics-smoke-${TEST_SUITE_ID}@test.com`,
  ...
  describe('SMOKE-SHIP-01: Critical Path', () => {
